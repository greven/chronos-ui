...
import { SystemStyleObject, useTheme } from '../system'
import { Dict, get, runIfFn, createContext } from '../util'
...
export function useTransitionConfig(themeKey: string, props: Dict, classNames?: Dict<string>) {
  const theme = useTheme()
  const path = `components.${themeKey}.transition`

  const defaultConfig = get(theme, path)
  const configObjectOrFn = (props.transitionConfig || defaultConfig) as MaybeTransitionConfig
...
const [TransitionsProvider, useTransitions] = createContext<UseTransitionConfigReturn>({
  errorMessage:
