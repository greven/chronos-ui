...
import { addListener, ColorMode, getColorScheme, storage, syncBodyClassName } from './utils'
...
export function useColorModeState<T extends ColorModeOptions>(options?: T) {
  const [mode, setMode] = React.useState<ColorMode>(options?.initialColorMode || 'light')
